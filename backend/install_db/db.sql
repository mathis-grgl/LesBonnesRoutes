...
FOR EACH ROW
BEGIN
	UPDATE TRAJET SET nbPlacesRestantes = nbPlaces - (SELECT SUM(nbPlaces) FROM TRAJET_EN_COURS_PASSAGER WHERE idTrajet = NEW.idTrajet)
...
FOR EACH ROW
BEGIN
	UPDATE TRAJET SET nbPlacesRestantes = nbPlaces - (SELECT SUM(nbPlaces) FROM TRAJET_EN_COURS_PASSAGER WHERE idTrajet = OLD.idTrajet)
